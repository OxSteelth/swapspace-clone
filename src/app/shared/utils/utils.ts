 ...
 */
export function compareTokens(token0: Currency, token1: Currency): boolean {
  return token0?.code?.toLowerCase() === token1?.code?.toLowerCase() && token0?.network === token1?.network;
}
 ...
 */
export function subtractPercent(amount: number | BigNumber | string, percent: number | BigNumber | string): BigNumber {
